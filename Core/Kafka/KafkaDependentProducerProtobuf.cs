...
public class KafkaDependentProducerProtobuf<K, V> : IKafkaDependentProducerProtobuf<K, V> where V : IMessage<V>, new()
{
    IProducer<K, V>? kafkaHandle;
    private KafkaClientHandle _handle;

    public KafkaDependentProducerProtobuf(KafkaClientHandle handle)
    {
        _handle = handle;
    }
